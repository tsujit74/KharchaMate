...
export const getGroupSettlement = async (groupId: string) => {
  ...
  try {
    const res = await api.get(`/blance/groups/${groupId}/settlement`);
...
export const settlePayment = async (
  ...
  try {
    const res = await api.post(`/blance/groups/${groupId}/pay`, {
      to: toUserId,
      amount,
    });
    ...
    if (status === 403) throw new Error("FORBIDDEN");
    if (status === 400)
      throw new Error(err.response.data?.message || "INVALID_PAYMENT");

    throw new Error("PAYMENT_FAILED");
  }
};

export const confirmSettlement = async (settlementId: string) => {
  if (!settlementId?.trim()) {
    throw new Error("INVALID_SETTLEMENT");
  }

  try {
    const res = await api.patch(`/blance/settlements/${settlementId}/confirm`);
    ...
    if (status === 403) {
      throw new Error("ONLY_RECEIVER_CAN_CONFIRM");
    }
    ...
    if (status === 400) {
      throw new Error(err.response.data?.message || "INVALID_CONFIRMATION");
    }
...
export const getSettlementRequests = async () => {
  try {
    const res = await api.get("/blance/settlement-requests");

    return {
      pending: Array.isArray(res.data?.pending) ? res.data.pending : [],
      confirmed: Array.isArray(res.data?.confirmed) ? res.data.confirmed : [],
    };
  } catch (err: any) {
    if (!err.response) {
      throw new Error("NETWORK_ERROR");
    ...
    throw new Error(
      err.response.data?.message || "FAILED_FETCH_SETTLEMENT_REQUESTS",
    );
  }
};
...
export const getMyNetBalance = async () => {
  try {
    const res = await api.get("/blance/my-net-balance");
